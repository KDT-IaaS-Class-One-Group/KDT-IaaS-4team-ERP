...
export default function CTPPaymentList({ className, cartProductCount, prodDescription, prodImgUrl, cartIndex }: CTPPaymentListProps) {
    ...
    <li className={paymentListDefault} id={idToString}>
      ...
      <div className='ctpButton flex justify-center items-center gap-2'>
        ...
        <div className='flex flex-col justify-center items-center gap-0 w-7'>
          <button type="button" className='w-full flex justify-center items-center text-center' onClick={handleDecrease}>-</button>
          <button type="button" className='w-full flex justify-center items-center text-center' onClick={handleIncrease}>+</button>
        </div> 
        {/* 버튼 세로 묶음 */}
      </div>
      {/* 버튼 컴포넌트 */}
